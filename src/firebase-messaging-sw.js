...
self.addEventListener('push', (event) => {
  if (event.data) {
    ...
    const title = payload.title || payload.notification?.title || 'carShare';
    const body = payload.body || payload.notification?.body || 'You have a new notification';
    const route = payload.route || payload.url || payload.data?.route || '/';
    event.waitUntil(self.registration.showNotification(title, {
      body,
      icon: '/assets/carShare-logo.png',
      badge: '/assets/carShare-logo.png',
      tag: payload.tag || 'carshare-notification',
      data: { url: route }
    }));
  }
});
