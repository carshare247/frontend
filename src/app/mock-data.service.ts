...
export class MockDataService {
  ...
  getRides(filters: { from?: string; to?: string; date?: string; passengers?: number; status?: string } = {}): Observable<Ride[]> {
    ...
    return this.http.get<ApiResponse<any[]>>(`${this.apiUrl}/rides`, { params }).pipe(
      map((response: any) => this.asItems(response)
        .map((ride: any) => this.fromApiRide(ride))
        .filter((ride: Ride) => this.isRideDateAllowed(ride.date)))
    );
  }
  ...
  private fromApiRide(ride: any): Ride {
    return { ...ride, from: ride.from ?? ride.fromLocation, to: ride.to ?? ride.toLocation,
      ...
      price: Number(ride.price), totalSeats: Number(ride.totalSeats ?? ride.seatsAvailable ?? 0), seatsAvailable: ride.availableSeats ?? ride.seatsAvailable,
  ...
  private fromApiOwner(owner: any): Owner {
    ...
    const profilePhoto = this.profilePhotoUrl(owner?.profilePhotoUrl);
