...
export class OwnerComponent {
  ...
  constructor(
    ...
    if (Capacitor.isNativePlatform()) {
      Browser.addListener('browserFinished', () => this.refreshVerificationStatus()).then(listener => this.browserFinishedListener = listener);
    }
  ...
  async verifyOtp() {
    ...
    try {
      ...
      if (!userId) {
        this.toast.show('Mobile verified, but the account could not be identified.', 'error');
        return;
      }
      this.mobileVerificationService.verifyMobileOnBackend(userId, result.firebaseUid, this.mobile).subscribe({
        next: () => {
          this.mobileVerified = true;
          this.toast.show('Mobile verified successfully', 'success');
        },
        error: () => this.toast.show('Mobile verification could not be saved. Please try again.', 'error')
      });
    } catch (error: any) {
