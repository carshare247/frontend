...
export class MultiStopBookingComponent implements OnInit {
  ...
  onSubmit() {
    ...
    const seats = Number(this.bookingForm.get('seats')?.value || 0);
    if (seats > this.availableSeats) {
      this.errorMessage = `Only ${this.availableSeats} seat(s) are available for this journey. Please reduce the number of seats.`;
      this.toast.show(this.errorMessage, 'error');
      return;
    }
    this.rideService.bookSegment({
      rideId: this.rideId,
      passengerId: currentUser.id,
      passengerMobile: currentUser.mobile || this.bookingForm.get('passengerMobile')?.value,
      ...
      next: (booking) => {
        ...
        this.bookingConfirmation = {
          ...
          seatsBooked: seats,
          totalAmount: booking.price ? Number(booking.price) * seats : this.getTotalPrice(),
          status: 'Pending'
        };
      ...
      error: (err) => {
        this.isSubmitting = false;
        this.errorMessage = err.error?.message || 'Booking failed. Complete any required rating before booking another ride.';
        this.toast.show(this.errorMessage, 'error');
      }
    });
  }
