...
export class MultiStopRideCreateComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (String(this.rideForm.get('date')?.value || '') < this.today) {
      this.errorMessage = 'Ride date must be today or later';
      return;
    }
    ...
    this.rideService.createMultiStopRide(request).subscribe({
      ...
      error: (err) => {
        this.isSubmitting = false;
        this.errorMessage = err.error?.message || 'Unable to create ride. Complete or cancel your active ride before posting another ride.';
