...
export class MultiStopRideService {
  ...
  searchRides(request: RideSearchRequest): Observable<RideSearchResponse> {
    ...
    let params = new HttpParams()
      ...
      .set('includeFull', String(request.includeFull ?? false));
    ...
    return new Observable(observer => {
      this.http.get<any>(`${this.apiUrl}/search/multi-stop`, { params })
        .subscribe({
          next: (response) => {
            const results = this.normalizeSearchResponse(response?.data ?? response);
  ...
  searchRidesPost(request: RideSearchRequest): Observable<RideSearchResponse> {
    ...
    return new Observable(observer => {
      const params = new HttpParams().set('includeFull', String(request.includeFull ?? false));
      this.http.post<any>(`${this.apiUrl}/search/multi-stop`, request, { params })
        .subscribe({
          next: (response) => {
            const results = this.normalizeSearchResponse(response?.data ?? response);
            this.isLoading.set(false);
  ...
  private normalizeSearchResponse(response: any): RideSearchResponse {
